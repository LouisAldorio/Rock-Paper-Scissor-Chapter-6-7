...
class GameHistoryAdapter : RecyclerView.Adapter<GameHistoryAdapter.GameHistoryHolder>() {
    ...
    private var items: MutableList<PlayerWithGameHistory> = mutableListOf()


    fun setItems(items: List<PlayerWithGameHistory>) {
        clearItems()
        addItems(items)
        notifyDataSetChanged()
    }

    fun addItems(items: List<PlayerWithGameHistory>) {
    ...
    class GameHistoryHolder(private val binding: CardWinnerItemBinding) :
        ...
        fun bindView(item: PlayerWithGameHistory) {
            binding.tvWinnerPlayerName.text = item.player1.name.uppercase() + " " + GameResult.get(
